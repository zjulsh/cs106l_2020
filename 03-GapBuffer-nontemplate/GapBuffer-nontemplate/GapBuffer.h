...
GapBuffer::GapBuffer() : _logical_size(0), _buffer_size(kDefaultSize), _cursor_index(0), _gap_size(kDefaultSize){
    _elems = new value_type[kDefaultSize];
}

GapBuffer::GapBuffer(size_type count, const value_type& val) : _logical_size(count), _buffer_size(2*count), _cursor_index(count), _gap_size(count){
...
typename GapBuffer::reference GapBuffer::operator[](size_type pos){
    return const_cast<reference>(
                static_cast<const GapBuffer*> (this)->operator [](pos)
                );
}
...
bool operator==(const GapBuffer& left, const GapBuffer& right){
    ...
    for (GapBuffer::size_type i = 0; i < left.size(); i++){
