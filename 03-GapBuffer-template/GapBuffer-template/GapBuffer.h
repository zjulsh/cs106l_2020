...
template <typename T>
GapBuffer<T>::GapBuffer() : _logical_size(0), _buffer_size(kDefaultSize), _cursor_index(0), _gap_size(kDefaultSize){
    _elems = new value_type[kDefaultSize];
}

template <typename T>
GapBuffer<T>::GapBuffer(size_type count, const value_type& val) : _logical_size(count), _buffer_size(2*count), _cursor_index(count), _gap_size(count) {
    _elems = new value_type[2*count];
...
typename GapBuffer<T>::reference GapBuffer<T>::at(size_type pos) {
    return const_cast<reference>(
                static_cast<const GapBuffer<T>*> (this)->at(pos)
                );
}
...
typename GapBuffer<T> ::const_reference GapBuffer<T>::at(size_type pos) const {
    if (pos >= _logical_size){
        throw ("external_index: array_index is out of bounds!");
    }

    size_type index = pos < _cursor_index ? pos : pos + _gap_size;
    return _elems[index];
}
...
typename GapBuffer<T>::reference GapBuffer<T>::operator[](size_type pos) {
    return const_cast<reference>(
                static_cast<const GapBuffer<T>*> (this)->operator [](pos)
                );
}
...
std::ostream& operator<<(std::ostream& os, const GapBuffer<T>& buf) {
    ...
    os << '{';
    typename GapBuffer<T>::size_type i = 0;
...
bool operator==(const GapBuffer<T>& lhs, const GapBuffer<T>& rhs) {
    ...
    for (typename GapBuffer<T>::size_type i = 0; i < lhs.size(); i++){
...
GapBufferIterator<T> GapBufferIterator<T>::operator++(int) {
    ...
    return iterator(this->_pointee, i);
...
template <typename T>
GapBuffer<T>::GapBuffer(std::initializer_list<T> init) : _logical_size(init.size()), _buffer_size(2*_logical_size), _cursor_index(_logical_size), _gap_size(_logical_size), _elems(new T[_buffer_size]) {
    // TODO: implement this initializer list constructor (~2 lines long)
    std::copy(init.begin(), init.end(), _elems);
}

template <typename T>
GapBuffer<T>::GapBuffer(const GapBuffer& other) : _logical_size(other._logical_size), _buffer_size(other._buffer_size), _cursor_index(other._cursor_index), _gap_size(other._gap_size){
    // TODO: implement this copy constructor (~4 lines long)
    // use member initialization list!
    _elems = new T[_buffer_size];

    for (auto i = 0; i < other._buffer_size; i++){
...
GapBuffer<T>& GapBuffer<T>::operator=(const GapBuffer& rhs) {
    ...
    for (auto i = 0; i < rhs._buffer_size; i++){
